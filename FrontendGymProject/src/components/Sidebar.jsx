...
const Sidebar = () => {
  ...
  const activeLink =
    'flex items-center gap-5 pl-4 pt-3 pb-2.5 rounded-lg text-md m-2 bg-light-gray';
  const normalLink =
    'flex items-center gap-5 pl-4 pt-3 pb-2.5 rounded-lg text-md text-gray-700 hover:bg-light-gray m-2';

  const activeList =
    'flex items-center gap-3 pl-4 pt-2 pb-2 rounded-lg text-md m-1 bg-light-gray';
  const normalList =
    'flex items-center gap-3 pl-4 pt-2 pb-2 rounded-lg text-md text-gray-700 hover:bg-light-gray m-1';
              ...
              <div key={item.title}>
                ...
                {item.links.map((link, index) => (
                  <div key={index}>
                    ...
                    isClicked.Subscribers === true ? (
                      <div className=' rounded-lg text-md ml-8 mr-7'>
                        {listAct.map((list, index) => (
                          <NavLink
                            to={`/${link.name}/${list.name}`}
                            key={index}
                            onClick={() => {
                              handleCloseSideBar();
                            }}
                            className={({ isActive }) =>
                              isActive ? activeList : normalList
                            }
                          >
                            <TbPointFilled />
